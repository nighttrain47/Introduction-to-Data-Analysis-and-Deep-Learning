# -*- coding: utf-8 -*-
"""
Phần 3 - Câu 7: Trực quan dữ liệu số lượng thí sinh đậu rớt dựa trên từng nhóm giới tính
"""
...
print("="*70)
print("TRỰC QUAN SỐ LƯỢNG THÍ SINH ĐẬU/RỚT TRÊN TỪNG NHÓM GIỚI TÍNH")
print("="*70)

# Ánh xạ giới tính
gt_labels = {'F': 'Nữ', 'M': 'Nam'}
df['GT_Label'] = df['GT'].map(gt_labels)

# Bảng chéo
crosstab = pd.crosstab(df['GT_Label'], df['KQXT'])
crosstab.columns = ['Rớt (0)', 'Đậu (1)']
print("\n--- Bảng chéo: Số lượng đậu/rớt theo Giới tính ---")
print(crosstab)
...
gt_list = crosstab.index.tolist()
x = np.arange(len(gt_list))
width = 0.35
...
axes[0].set_title('Số lượng Đậu/Rớt theo Giới tính', fontsize=14, fontweight='bold')
axes[0].set_xticks(x)
axes[0].set_xticklabels(gt_list, fontsize=11)
axes[0].legend()
...
bars3 = axes[1].bar(gt_list, rot_percent, label='Rớt', color='#e74c3c', edgecolor='black')
bars4 = axes[1].bar(gt_list, dau_percent, bottom=rot_percent, label='Đậu', color='#2ecc71', edgecolor='black')

axes[1].set_xlabel('Giới tính', fontsize=12)
axes[1].set_ylabel('Tỷ lệ (%)', fontsize=12)
axes[1].set_title('Tỷ lệ Đậu/Rớt theo Giới tính (Stacked 100%)', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('DuLieu/Phan3_Cau7_KQXT_by_GT.png', dpi=300, bbox_inches='tight')
plt.show()

print("\nĐã lưu biểu đồ: DuLieu/Phan3_Cau7_KQXT_by_GT.png")
