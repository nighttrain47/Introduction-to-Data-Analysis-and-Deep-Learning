# -*- coding: utf-8 -*-
"""
Phần 3 - Câu 5: Trực quan dữ liệu số lượng thí sinh đậu rớt trên từng nhóm khu vực
"""
...
print("="*70)
print("TRỰC QUAN SỐ LƯỢNG THÍ SINH ĐẬU/RỚT TRÊN TỪNG NHÓM KHU VỰC")
print("="*70)

# Bảng chéo
crosstab = pd.crosstab(df['KV'], df['KQXT'])
crosstab.columns = ['Rớt (0)', 'Đậu (1)']
print("\n--- Bảng chéo: Số lượng đậu/rớt theo Khu vực ---")
print(crosstab)
...
kv_list = crosstab.index.tolist()
x = np.arange(len(kv_list))
width = 0.35
...
axes[0].set_title('Số lượng Đậu/Rớt theo Khu vực', fontsize=14, fontweight='bold')
axes[0].set_xticks(x)
axes[0].set_xticklabels(kv_list, fontsize=11)
axes[0].legend()
...
bars3 = axes[1].bar(kv_list, rot_percent, label='Rớt', color='#e74c3c', edgecolor='black')
bars4 = axes[1].bar(kv_list, dau_percent, bottom=rot_percent, label='Đậu', color='#2ecc71', edgecolor='black')

axes[1].set_xlabel('Khu vực', fontsize=12)
axes[1].set_ylabel('Tỷ lệ (%)', fontsize=12)
axes[1].set_title('Tỷ lệ Đậu/Rớt theo Khu vực (Stacked 100%)', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('DuLieu/Phan3_Cau5_KQXT_by_KV.png', dpi=300, bbox_inches='tight')
plt.show()

print("\nĐã lưu biểu đồ: DuLieu/Phan3_Cau5_KQXT_by_KV.png")
