# -*- coding: utf-8 -*-
"""
Phần 3 - Câu 6: Trực quan dữ liệu số lượng thí sinh đậu rớt dựa trên từng nhóm dân tộc
"""
...
print("="*70)
print("TRỰC QUAN SỐ LƯỢNG THÍ SINH ĐẬU/RỚT TRÊN TỪNG NHÓM DÂN TỘC")
print("="*70)

# Ánh xạ mã dân tộc
dt_labels = {0.0: 'Kinh', 1.0: 'DT khác (1)', 6.0: 'DT khác (6)'}
df['DT_Label'] = df['DT'].map(dt_labels)

# Bảng chéo
crosstab = pd.crosstab(df['DT_Label'], df['KQXT'])
crosstab.columns = ['Rớt (0)', 'Đậu (1)']
print("\n--- Bảng chéo: Số lượng đậu/rớt theo Dân tộc ---")
print(crosstab)
...
axes[0].set_title('Số lượng Đậu/Rớt theo Dân tộc', fontsize=14, fontweight='bold')
axes[0].set_xticks(x)
axes[0].set_xticklabels(dt_list, fontsize=11)
axes[0].legend()
...
bars3 = axes[1].bar(dt_list, rot_percent, label='Rớt', color='#e74c3c', edgecolor='black')
bars4 = axes[1].bar(dt_list, dau_percent, bottom=rot_percent, label='Đậu', color='#2ecc71', edgecolor='black')

axes[1].set_xlabel('Dân tộc', fontsize=12)
axes[1].set_ylabel('Tỷ lệ (%)', fontsize=12)
axes[1].set_title('Tỷ lệ Đậu/Rớt theo Dân tộc (Stacked 100%)', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('DuLieu/Phan3_Cau6_KQXT_by_DT.png', dpi=300, bbox_inches='tight')
plt.show()

print("\nĐã lưu biểu đồ: DuLieu/Phan3_Cau6_KQXT_by_DT.png")
